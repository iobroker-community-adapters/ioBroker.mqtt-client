            ...
            it('should configure and start adapter', function () {
                return new Promise(async (resolve, reject) => {
                    try {
                        ...
                        // Verify states
                        const stateIds = await harness.dbConnection.getStateIDs('mqtt-client.0.*');
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (!connectionState || connectionState.val === false) {
                console.log('✅ Connection state correctly shows disconnected');
                resolve(true);
            } else {
                reject(new Error('Expected connection state to be false for invalid broker'));
            }
...
For the MQTT client adapter, create specific tests for broker connectivity and message flow:
...
```javascript
it("Should connect to MQTT broker with test credentials", async () => {
    await harness.changeAdapterConfig("mqtt-client", {
        native: {
            host: "test.mosquitto.org",
            port: 1883,
    ...
    if (connectionState?.val === true) {
        ...
        throw new Error("MQTT Test Failed: Expected connection to be established. Check logs for broker errors.");
    }
}).timeout(30000);
```
...
For MQTT client functionality, follow these patterns when implementing or modifying the adapter:
...
/**
 * Converts MQTT topic to ioBroker object ID
 * @param {string} topic - MQTT topic path
 * @returns {string} ioBroker object ID
 */
...
If using TypeScript, ensure proper type definitions for MQTT client configuration:
